...
export const BADGE_DEFINITIONS = [
  ...
  { id: 'consistent',   name: 'Consistent',       icon: '💪', description: 'Complete all rituals in a single day.' },
  { id: 'trusted',      name: 'Trusted',          icon: '📋', description: 'Complete 10 tasks assigned by others.' },
  { id: 'unstoppable',  name: 'Unstoppable',      icon: '🏆', description: 'Maintain a 30-day streak on any ritual.' },
  ...
  { id: 'perfect_week', name: 'Perfect Week',     icon: '🎯', description: 'Complete all rituals every day for 7 days.' },
  // New
  { id: 'task_5',       name: 'Getting Started',  icon: '📌', description: 'Complete 5 tasks.' },
  { id: 'task_25',      name: 'Task Master',      icon: '🗂️', description: 'Complete 25 tasks.' },
  { id: 'task_50',      name: 'Powerhouse',       icon: '⚙️', description: 'Complete 50 tasks.' },
  { id: 'streak_7',     name: 'Week Warrior',     icon: '🗓️', description: 'Maintain a 7-day streak on any ritual.' },
  { id: 'streak_14',    name: 'Two Weeks Strong', icon: '📅', description: 'Maintain a 14-day streak on any ritual.' },
  { id: 'goal_getter',  name: 'Goal Getter',      icon: '🎖️', description: 'Complete your first goal.' },
  { id: 'multitasker',  name: 'Multitasker',      icon: '🔀', description: 'Complete 3 tasks in a single day.' },
  { id: 'comeback_kid', name: 'Comeback Kid',     icon: '🔄', description: 'Return and complete a task after a 7-day gap.' },
  { id: 'ritualist',    name: 'Ritualist',        icon: '🧘', description: 'Complete any ritual 50 times total.' },
  { id: 'overachiever', name: 'Overachiever',     icon: '🚀', description: 'Complete all rituals for 30 consecutive days.' },
];
...
export const checkAndUnlockBadges = () => {
  try {
    ...
    if (!unlockedIds.has('multitasker')) {
      const tasksDoneToday = completionLog.filter(l => l.type === 'task' && getLocalYMD(new Date(l.timestamp)) === todayStr);
      if (tasksDoneToday.length >= 3)
        newlyEarned.push(BADGE_DEFINITIONS.find(b => b.id === 'multitasker'));
    }

    if (!unlockedIds.has('comeback_kid')) {
      const taskLogs = completionLog.filter(l => l.type === 'task').sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
      for (let i = 1; i < taskLogs.length; i++) {
        const gap = (new Date(taskLogs[i].timestamp) - new Date(taskLogs[i-1].timestamp)) / (1000 * 60 * 60 * 24);
        if (gap >= 7) { newlyEarned.push(BADGE_DEFINITIONS.find(b => b.id === 'comeback_kid')); break; }
      }
    }

    if (!unlockedIds.has('ritualist')) {
      const habitCounts = {};
      completionLog.filter(l => l.type === 'habit').forEach(l => { habitCounts[l.id] = (habitCounts[l.id] || 0) + 1; });
    ...
    if (!unlockedIds.has('overachiever') && habits.length > 0) {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        const ymd = getLocalYMD(d);
        const done = completionLog.filter(l => l.type === 'habit' && getLocalYMD(new Date(l.timestamp)) === ymd);
        if (habits.every(h => done.some(l => l.id === h.id))) { perfectDays++; } else { break; }
      }
      if (perfectDays >= 30) newlyEarned.push(BADGE_DEFINITIONS.find(b => b.id === 'overachiever'));
    }
