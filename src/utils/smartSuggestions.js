...
function rule1_consistentMissDay(habits, completionLog) {
  ...
  for (const habit of habits) {
    ...
    const logs = logsFor(completionLog, habit.id).filter(
      (l) => l.timestamp >= fourWeeksAgo,
    );
    const doneDays = new Set(logs.map((l) => dayStr(l.timestamp)));

    for (const ds of doneDays) {
      const dow = new Date(ds).getDay();
      buckets[dow].completed = Math.min(
        buckets[dow].completed + 1,
        buckets[dow].total,
      );
    }

    const rates = buckets.map((b) =>
      b.total > 0 ? b.completed / b.total : null,
    );
...
function rule2_streakRecovery(habits, completionLog) {
  ...
  for (const habit of habits) {
    ...
    const doneDays = [
      ...new Set(recentLogs.map((l) => dayStr(l.timestamp))),
    ].sort();
...
function rule3_lateSleep(habits, completionLog) {
  const sleepHabit = habits.find(
    (h) => !h.paused && h.type === "time_locked" && /sleep/i.test(h.name),
  );
...
function rule4_waterAfternoon(habits, completionLog) {
  const waterHabit = habits.find(
    (h) => !h.paused && /water|drink/i.test(h.name),
  );
  if (!waterHabit) return [];
  if ((waterHabit.count || 0) >= (waterHabit.goal || 1)) return [];

  const sevenDaysAgo = daysAgo(7);
  const recentLogs = logsFor(completionLog, waterHabit.id).filter(
    (l) => l.timestamp >= sevenDaysAgo,
  );
  if (recentLogs.length < 5) return [];

  const allBeforeNoon = recentLogs.every(
    (l) => new Date(l.timestamp).getHours() < 12,
  );
...
function rule7_goalBehindRitual(habits, completionLog) {
  ...
  let goals = [];
  try {
    goals = JSON.parse(localStorage.getItem("pinboard_goals") || "[]");
  } catch (e) {}
      ...
      (h) =>
        ...
        ((goal.linkedHabitId && h.id === goal.linkedHabitId) ||
          goal.name
            .toLowerCase()
            .split(" ")
            .some((w) => w.length > 3 && h.name.toLowerCase().includes(w))),
    );
    if (!linkedHabit) continue;

    const weekDays = completedDaysInRange(
      completionLog,
      linkedHabit.id,
      daysAgo(7),
    );
...
function rule8_overdueTasks(tasks) {
  ...
  const overdueTasks = tasks.filter(
    (t) => !t.done && t.dueDate && new Date(t.dueDate) < now,
  );
  ...
  try {
    ...
    if (!existing.includes(id)) {
      ...
      localStorage.setItem(
        "pinboard_dismissed_suggestions",
