...
export default function RewardsSection() {
  ...
  const handleShareClick = () => {
    ...
    const pointsThisWeek = weekLogs.reduce(
      (sum, l) => sum + (l.type === "task" ? 10 : 5),
      0,
    );
    ...
    const habitsDone = Object.values(habitDayMap).reduce(
      (acc, s) => acc + s.size,
      0,
    );
  ...
  if (nextLevel) {
    ...
    progressPercentage = Math.min(
      100,
      Math.max(0, (progressIntoLevel / range) * 100),
    );
    ...
    <div className="w-full max-w-md z-10 flex flex-col gap-6 pt-6 pb-8">
      ...
      <div className="bg-gradient-to-br from-indigo-900 to-purple-900 rounded-2xl p-6 mb-8 text-center shadow-[0_4px_20px_rgba(79,70,229,0.3)] border border-indigo-500/30 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          {nextLevel ? (
            <p className="text-xs text-indigo-200 font-medium mb-3.5">
              {pointsToNext.toLocaleString()} pts to {nextLevel.name}
            </p>
          ) : (
            <p className="text-xs text-amber-300 font-bold uppercase tracking-widest mb-3.5">
              Max Level Reached!
            </p>
          )}
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-1.5 text-xs font-semibold text-[var(--accent-purple)] hover:text-[var(--text-primary)] bg-white/5 hover:bg-white/10 rounded-full border border-white/10 active:scale-95 transition-all"
          >
            ...
            return (
              <div
                ...
                className={`relative flex flex-col items-center text-center p-4 rounded-2xl transition-all duration-300 ${
                  isEarned
                    ? "bg-[var(--bg-card)] border border-amber-500/20 shadow-[0_0_15px_rgba(251,191,36,0.05)]"
                    : "bg-[var(--bg-card)] opacity-60 filter grayscale"
                }`}
              >
                ...
                <h4
                  className={`font-bold text-sm mb-1 ${isEarned ? "text-[var(--text-primary)]" : "text-[var(--text-secondary)]"}`}
                >
                  {badgeDef.name}
                </h4>
                <p className="text-[10px] text-[var(--text-muted)] leading-tight mb-3 flex-grow">
                ...
                {isEarned ? (
                  ...
                  <span className="text-[9px] font-medium text-[var(--text-muted)] bg-[var(--bg-card-hover)] px-2 py-1 rounded-md w-full">
                    Locked
                  </span>
                )}
              </div>
