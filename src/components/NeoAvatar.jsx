...
const BUBBLE_LINES = [
  "I'm Neo! 🧅",
  "Layers of productivity!",
  "Don't cry… or do, I'm Neo 😂",
  "Peeling great today! 🌟"
];

export default function NeoAvatar({ habits = [] }) {
  const [speech, setSpeech] = useState(null);
  const [bounce, setBounce] = useState(false);

  // Exclude paused habits (if paused property exists)
  ...
  if (pct === 1 && totalHabits > 0) {
    state = 5;
    defaultSpeech = "YESSS!! You crushed it!! 🎉";
    animationClass = "neo-proud-pulse";
  } else if (pct >= 0.7) {
    state = 4;
    defaultSpeech = "SO CLOSE!! One more!! 🔥";
    animationClass = "neo-energetic-bounce";
  } else if (pct >= 0.4) {
    state = 3;
    defaultSpeech = "Looking great! Almost there ⚡";
    animationClass = "neo-faster-float";
  } else if (pct > 0) {
    state = 2;
    defaultSpeech = "Good start, [name]! 💪";
    animationClass = "neo-gentle-float";
  }

  // Handle [name] replacement
  const userName = localStorage.getItem('pinboard_user_name') || 'friend';
  defaultSpeech = defaultSpeech.replace('[name]', userName);

  const speechTimeoutRef = useRef(null);

  // Initialize default speech when state changes
  useEffect(() => {
  ...
  // Confetti on state 5 transition
  ...
  const handleTap = () => {
    ...
    const randomLine = BUBBLE_LINES[Math.floor(Math.random() * BUBBLE_LINES.length)];
  ...
  return (
    <div className="fixed bottom-[80px] right-2 flex flex-col items-end justify-end select-none z-40 pointer-events-none">
      ...
      {/* Speech Bubble */}
      <div 
        className={`absolute bottom-full right-4 mb-2 bg-white text-gray-900 px-3 py-1.5 rounded-2xl text-xs font-bold shadow-lg max-w-[160px] text-center transition-opacity duration-300 pointer-events-none ${speech ? 'opacity-100' : 'opacity-0'}`}
      >
        {speech}
