...
const ShareCard = forwardRef(function ShareCard(
  ...
  const stats = [
    ...
    { emoji: '✅', label: 'Habits Done', value: `${habitsDone} / ${habitTotal}` },
    { emoji: '🏆', label: 'Level', value: levelName || 'Beginner' },
  ];

  function getHabitEmoji(name) {
    const n = (name || '').toLowerCase();
    if (n.includes('water') || n.includes('drink') || n.includes('hydrate')) return '💧';
    if (n.includes('gym') || n.includes('exercise') || n.includes('workout') || n.includes('run') || n.includes('walk') || n.includes('sport') || n.includes('yoga') || n.includes('stretch') || n.includes('lift') || n.includes('training')) return '🏃‍♂️';
    ...
    if (n.includes('meditat') || n.includes('mind') || n.includes('breathe') || n.includes('calm') || n.includes('zen')) return '🧘';
    if (n.includes('code') || n.includes('work') || n.includes('programming') || n.includes('dev') || n.includes('study')) return '💻';
    if (n.includes('journal') || n.includes('write') || n.includes('diary')) return '✍️';
    if (n.includes('eat') || n.includes('food') || n.includes('meal') || n.includes('diet') || n.includes('cook')) return '🥗';
    if (n.includes('clean') || n.includes('house') || n.includes('chore') || n.includes('tidy')) return '🧹';
  ...
  return (
    <div ref={ref} style={card}>
      ...
      <div style={neoWrapper}>
        <img src={neoImg} alt="Neo" style={{ width: 110, height: 110, objectFit: 'contain' }} />
      </div>
      ...
      <div style={grid}>
        {stats.map(s => (
          <div key={s.label} style={statBox}>
            <div style={statLabel}>{s.emoji} {s.label}</div>
      ...
      {mvpHabit && (
        <div style={mvpBox}>
          ...
          <div style={{ fontSize: 40, lineHeight: 1, marginTop: 4, marginBottom: 4 }}>{getHabitEmoji(mvpHabit.name)}</div>
      ...
      <div style={bottomBar}>
        <div style={neoCircle}>
          <img src={neoImg} alt="" style={{ width: 48, height: 48, objectFit: 'cover' }} />
        </div>
