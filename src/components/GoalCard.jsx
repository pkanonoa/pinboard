...
export default function GoalCard({ goal, onLog, onComplete, onUndo, onDelete, onEdit, onTogglePause }) {
  ...
  // Status Calculation
  const getStatus = () => {
    if (goal.isCompleted) return { label: 'Completed', color: 'text-[var(--success)] bg-emerald-900/30' };
    if (goal.paused) return { label: 'Paused', color: 'text-[var(--text-secondary)] bg-gray-800' };
    ...
    if (goal.dueDate) {
      ...
      if (today > due) {
        return { label: 'Overdue', color: 'text-[var(--danger)] bg-red-900/30' };
      }
      ...
      if (goal.trackingType !== 'binary' && goal.createdAt) {
        ...
        if (goal.progress >= expectedProgress) return { label: 'On track', color: 'text-[var(--success)] bg-emerald-900/30' };
        if (goal.progress >= expectedProgress * 0.75) return { label: 'At risk', color: 'text-[var(--warning)] bg-amber-900/30' };
      }
    }
    
    // Default fallback
    return { label: 'In progress', color: 'text-[var(--accent-purple)] bg-indigo-500/20' };
  };
    ...
    <div className={`rounded-2xl transition-all duration-300 overflow-hidden bg-[#1e1e28] select-none ${isExpanded ? 'shadow-xl shadow-black/20' : ''}`}>
      ...
      >
        <div className="flex justify-between items-start mb-3">
          <div className="pr-4">
            <h3 className={`font-medium text-base leading-tight mb-1 ${goal.isCompleted ? 'text-[var(--text-secondary)] line-through' : 'text-[var(--text-primary)]'}`}>
              {goal.name}
            </h3>
        ...
        <div className="px-4 pb-4 pt-4 border-t border-[#16161f] bg-[#1a1a24] animate-fade-in-down">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              {onTogglePause && (
                <button onClick={(e) => { e.stopPropagation(); onTogglePause(goal.id); }} className={`p-1.5 rounded-md transition-colors ${goal.paused ? 'text-[var(--warning)] bg-amber-900/20 hover:bg-amber-900/40' : 'text-[var(--text-secondary)] hover:text-[var(--warning)]'}`} title={goal.paused ? "Resume" : "Pause"}>
              ...
              <button onClick={(e) => { e.stopPropagation(); onEdit(goal); }} className="p-1.5 text-[var(--text-secondary)] hover:text-[var(--accent-purple)] transition-colors" title="Edit">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
              </button>
              <button onClick={(e) => { e.stopPropagation(); if(confirm('Delete this goal?')) onDelete(goal.id); }} className="p-1.5 text-[var(--text-secondary)] hover:text-[var(--danger)] transition-colors" title="Delete">
          ...
          <div className="flex flex-col gap-2">
            {goal.trackingType === 'binary' ? (
              goal.isCompleted ? (
                ...
                <button onClick={() => onComplete(goal.id)} className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-[var(--text-primary)] font-bold rounded-xl transition-all shadow-lg shadow-indigo-900/20 active:scale-95">
                  Mark Complete
                </button>
              )
            ) : (
                  ...
                  {goal.trackingType === 'count_toward' ? (
                    ...
                    <form onSubmit={handleLogSubmit} className="flex gap-2">
                      <input 
                        ...
                        disabled={goal.isCompleted}
                        className="flex-1 bg-[#16161f] border-none rounded-xl px-4 py-2 text-sm text-[var(--text-primary)] focus:ring-1 focus:ring-indigo-500 focus:outline-none disabled:opacity-50"
                      />
                      <button 
                        type="submit" 
                        disabled={goal.isCompleted || !logValue}
                        className="px-6 bg-indigo-500 hover:bg-indigo-600 disabled:bg-gray-700 text-[var(--text-primary)] font-bold rounded-xl transition-colors active:scale-95"
                      >
                        Log
                      </button>
                    </form>
